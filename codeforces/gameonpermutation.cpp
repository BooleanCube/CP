...
void solve() {
    ...
    lower.insert(perm[0]);
    fr(i, 1, n) {
        auto it = lower.lower_bound(perm[i]);
        auto it2 = lucky.lower_bound(perm[i]);
        if(it == lower.begin() || *(--it) >= perm[i]) {
            //not lucky
            if(perm[i]<perm[i-1]) lower.insert(perm[i]);
        } else{
            if(it2 != lucky.begin() && *(--it2) < perm[i]) continue;
            ans++;
            lucky.insert(perm[i]);
        }
    }
