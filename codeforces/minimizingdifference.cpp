...
void solve() {
    ...
    vll pref(n), suff(n);
    frn(i, n) {
        if(i < n-1) pref[i] = a[i+1] - a[i];
        if(i > 0) suff[i] = a[i] - a[i-1];
    }
    ll lp = 0, rp = n-1, diff = 0;
    while(lp < rp) {
        if(k < min(lp+1, n-rp)) break;
        if(lp+1 < n-rp) {
            ll x = min(pref[lp], k/(lp+1));
            pref[lp] -= x;
            k -= x*(lp+1);
            diff += x, lp++;
        } else {
            ll x = min(suff[rp], k/(n-rp));
            suff[rp] -= x;
            k -= x*(n-rp);
            diff += x, rp--;
        }
    }
    cout << max(0, a[n-1]-a[0]-diff) << endl;
}
