...
void solve() {
    ...
    map<pii, pair<int, mii>> pref, mid, suff;
    ll ans = 0;
    frn(i, n-2) {
        pii cur = {a[i], a[i+1]};
        ans += pref[cur].f - pref[cur].s[a[i+2]];
        pref[cur].f++; pref[cur].s[a[i+2]]++;
    }
    frn(i, n-2) {
        pii cur = {a[i+1], a[i+2]};
        ans += suff[cur].f - suff[cur].s[a[i]];
        suff[cur].f++; suff[cur].s[a[i]]++;
    }
    frn(i, n-2) {
        pii cur = {a[i], a[i+2]};
        ans += mid[cur].f - mid[cur].s[a[i+1]];
        mid[cur].f++; mid[cur].s[a[i+1]]++;
    }
    cout << ans << endl;
}
