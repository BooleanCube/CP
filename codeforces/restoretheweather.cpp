...
void solve() {
    int n, k; cin >> n >> k;
    vector<pair<int, int>> weather(n);
    multiset<int> real;
    frn(i, n) { cin >> weather[i].first; weather[i].second = i; }
    frn(i, n) { int t; cin >> t; real.insert(t); }
    sort(weather.begin(), weather.end());
    vector<int> out(n);
    frn(i, n) {
        auto it = real.lower_bound(weather[i].first-k);
        if(it == real.end() || *it > weather[i].first+k) {
            cout << -1 << endl;
            return;
        }
        out[weather[i].second] = *it;
        real.erase(it);
    }
    printv(out);
}
