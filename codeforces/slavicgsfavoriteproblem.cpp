...
void dfs1(int cur, int par, int x, si &paths) {
    if(cur == b) return;
    paths.insert(x);
    for(auto [nbr, wt]: graph[cur]) {
        if(nbr == par) continue;
        dfs1(nbr, cur, x ^ wt, paths);
    }
}

bool dfs2(int cur, int par, int x, si &paths) {
    if(cur != b && paths.count(x)) return 1;
    for(auto [nbr, wt] : graph[cur]) {
        if(nbr == par) continue;
        if(dfs2(nbr, cur, x ^ wt, paths)) return 1;
    } 
    return 0;
}
...
void solve() {
    ...
    frn(i, n-1) {
        ...
        graph[aa].push_back({bb, w});
        graph[bb].push_back({aa, w});
    }
    set<int> ap;
    dfs1(a, -1, 0, ap);
    cout << (dfs2(b, -1, 0, ap) ? "YES" : "NO") << endl;
}
