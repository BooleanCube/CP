...
int dfs1(int u, int p, int dest, vi &bb) {
    if(u == dest) return 1;
    int ans = 0;
    for(int v : tree[u]) {
        if(v == p) continue;
        bb[v] = u;
        ans = max(ans, dfs1(v, u, dest, bb)+1);
    }
    return ans;
}

ll dfs2(int u, int p) {
    if(sz(tree[u]) == 1 && tree[u][0] == p) return 0;
    ll ans = 0;
    for(int v : tree[u]) {
        if(v == p) continue;
        ans += dfs2(v, u) + 2;
...
void solve() {
    ...
    int root = path[sz(path) >> 1];
    ll ans = dfs2(root, -1) - height(root, -1) + ((pl+1) >> 1) - 1;
    cout << ans << endl;
}
