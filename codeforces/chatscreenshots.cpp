...
vi topoSort(const vector<vi>& gr) {
    ...
    fr(i, 0, sz(gr)) if (indeg[i] == 0) q.push(i);
    while (!q.empty()) {
...
void solve() {
    cin >> n >> k;
    vector<vi> graph = vector<vi>(n, vi(0));
    frn(i, k) {
        vi ss(n);
        frn(j, n) {
            cin >> ss[j];
        ...
        fr(j, 2, n) graph[ss[j-1]].push_back(ss[j]);
    }
    // printv(topoSort(graph));
    cout << (sz(topoSort(graph)) == n ? "YES" : "NO") << endl;
}
